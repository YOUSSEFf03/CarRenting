...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const DesAscContainer = document.getElementById("DesAscContainer");
    // const ascendantDescendantSort = document.getElementById('ascendantDescendantSort');
    let sort_btn_press = "";
    ...
    function sortByPriceDes(cars) {
        return cars.slice().sort((a, b) => b.price - a.price);
    }

    function sortByYearDes(cars) {
    ...
    sortByPriceBtn.addEventListener("click", function () {
        sort_btn_press = "Price";
        clear_btn.style.display = "block";

        fetch("cars.json")
            .then(response => response.json())
            .then(data => {
                const sortOrder = DesAscButton.textContent;
                let sortedCars;

                if (sortOrder === "Descendant") {
                    sortedCars = sortByPriceDes(data);
                } else {
                    sortedCars = sortByPriceAsc(data);
                }
    ...
    sortByYearBtn.addEventListener("click", function () {
        ...
        fetch("cars.json")
            ...
            .then(data => {
                ...
                if (sortOrder === "Descendant") {
                    sortedCars = sortByYearDes(data);
                } else {
                    sortedCars = sortByYearAsc(data);
                }
    ...
    DesAscContainer.addEventListener("click", function () {

        clear_btn.style.display = "block";

        const DesAscButton = document.getElementById("DesAscButton");
        const arrow_UP_DOWN = document.getElementById("arrow_UP_DOWN");


        const sortOrder = DesAscButton.textContent;
        DesAscButton.textContent = "dd";


        if (sortOrder === "Descendant") {
            DesAscButton.textContent = "Ascendant";
            arrow_UP_DOWN.style.transform = "rotate(180deg)";
        } else {
            DesAscButton.textContent = "Descendant";
            arrow_UP_DOWN.style.transform = "rotate(0deg)";
        }

        if (sort_btn_press === "Price") {
            sortByPriceBtn.click();
        } else if (sort_btn_press === "Year") {
            sortByYearBtn.click();
        }
        else {
            // sortAlphabetic();
...
dropdowns.forEach(dropdown => {
    const sort_select = dropdown.querySelector('#sort-select');
    const sort_icon = dropdown.querySelector('.sort_icon');
    const sort_menu = dropdown.querySelector('.sort-menu');

    sort_select.addEventListener('click', () => {
        sort_menu.classList.toggle('sort-menu-open');
        sort_icon.classList.toggle('rotate-sort');
    });
    ...
    document.addEventListener('click', (event) => {
        ...
        if (!isClickInside) {
            ...
            sort_icon.classList.remove('rotate-sort');
        }
    });
});
