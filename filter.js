...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const submit_btn = document.getElementById("submit_filters_btn");
...
function filter() {
    ...
    const checkboxesBrand = document.querySelectorAll('input[name="CarBrand"]');
    const filteredCarsBrand = Array.from(checkboxesBrand)
        .filter(cb => cb.checked)
        .map(cb => cb.id.slice(0, -6));
    ...
    const checkboxesType = document.querySelectorAll('input[name="CarType"]');
    const filteredCarsType = Array.from(checkboxesType)
        .filter(cb => cb.checked)
        .map(cb => cb.id.slice(0, -5));

    const checkboxesFuel = document.querySelectorAll('input[name="FuelType"]');
    const filteredCarsFuel = Array.from(checkboxesFuel)
        .filter(cb => cb.checked)
        .map(cb => cb.id.slice(0, -5));

    const checkboxesTransmission = document.querySelectorAll('input[name="Transmission_type"]');
    const filteredCarsTransmission = Array.from(checkboxesTransmission)
        .filter(cb => cb.checked)
        .map(cb => cb.id.slice(0, -5));
    ...
    fetch("cars.json")
        ...
        .then(data => {
            let filteredCars = data.filter(car => {
                const brandCondition = filteredCarsBrand.length === 0 || filteredCarsBrand.includes(car.make);
                const priceCondition = car.price >= minPrice && car.price <= maxPrice;
                const yearCondition = car.year >= minYear && car.year <= maxYear;
                const typeCondition = filteredCarsType.length === 0 || filteredCarsType.includes(car['car-type']);
                const fuelCondition = filteredCarsFuel.length === 0 || filteredCarsFuel.includes(car['fuel-type']);
                const transmissionCondition = filteredCarsTransmission.length === 0 || filteredCarsTransmission.includes(car.transmission);
                return brandCondition && priceCondition && yearCondition && typeCondition && fuelCondition && transmissionCondition;
            });
